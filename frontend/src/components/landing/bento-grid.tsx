'use client';

import { cn } from '@/lib/utils';
import { motion } from 'framer-motion';
import {
  ArrowRight,
  BookOpen,
  Calendar,
  Code,
  FileText,
  Layers,
  Palette,
  PieChart,
  Terminal,
  Upload,
  Zap,
} from 'lucide-react';
import { ReactNode } from 'react';

interface BentoGridItemProps {
  title: string;
  description: string;
  icon: ReactNode;
  className?: string;
  size?: 'small' | 'medium' | 'large';
}

const iconVariants = {
  initial: { rotate: 0, scale: 1 },
  animate: {
    rotate: [0, 10, -10, 0],
    scale: [1, 1.05, 1],
    transition: {
      duration: 2,
      repeat: Infinity,
      repeatDelay: 1.5,
      ease: 'easeInOut' as const,
    },
  },
};

const BentoGridItem = ({
  title,
  description,
  icon,
  className,
  size = 'small',
}: BentoGridItemProps) => {
  const cardVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: {
      opacity: 1,
      y: 0,
      transition: { type: "spring" as const, damping: 25 },
    },
  };

  return (
    <motion.div
      variants={cardVariants}
      className={cn(
        'group relative flex h-full cursor-pointer flex-col justify-between overflow-hidden rounded-2xl border border-border bg-card px-6 pt-6 pb-8 shadow-md transition-all duration-500 hover:shadow-xl',
        className,
      )}
    >
      {/* Background grid effect */}
      <div className="absolute top-0 -right-1/2 z-0 size-full bg-[linear-gradient(to_right,#3d16165e_1px,transparent_1px),linear-gradient(to_bottom,#3d16165e_1px,transparent_1px)] bg-[size:24px_24px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)]" />

      {/* Big background icon */}
      <div className="absolute right-2 bottom-3 text-primary group-hover:text-primary opacity-20 hover:opacity-15 z-0 scale-[6] transition-all duration-500 group-hover:scale-[6.5]">
        <motion.div variants={iconVariants} initial="initial">
          {icon}
        </motion.div>
      </div>

      {/* Main content */}
      <div className="relative z-10 flex flex-col justify-between h-full">
        <div>
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary shadow transition-all duration-500 group-hover:bg-primary/20">
            <motion.div variants={iconVariants} initial="initial">
              {icon}
            </motion.div>
          </div>
          <h3 className="text-xl font-semibold text-foreground mb-1">{title}</h3>
          <p className="text-muted-foreground text-sm leading-snug">
            {description}
          </p>
        </div>
        <div className="mt-4 flex items-center text-sm font-medium text-primary">
          <span>Learn more</span>
          <ArrowRight className="ml-1 size-4 transition-transform duration-300 group-hover:translate-x-1" />
        </div>
      </div>

      {/* Bottom glow */}
      <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-primary to-primary/30 blur-2xl transition-all duration-500 group-hover:blur-lg" />
    </motion.div>
  );
};

const items = [
  {
    title: 'Transcript Upload',
    description:
      'Skip manual entry, just upload your UMD transcript and auto-import all courses, credits, and gen eds instantly',
    icon: <Upload className="size-6" />,
    size: 'large' as const,
  },
  {
    title: 'GenEd Tracking',
    description:
      'See which gen ed requirements you\'ve completed and what\'s still missing',
    icon: <BookOpen className="size-6" />,
    size: 'small' as const,
  },
  {
    title: 'CS Concentration',
    description:
      'Easily track your upper-level concentration path to ensure you\'re meeting all major requirements',
    icon: <Terminal className="size-6" />,
    size: 'medium' as const,
  },
  {
    title: 'Advisor-Ready PDF',
    description:
      'Export the mandatory, standard PDF version of your plan to share directly with your academic advisor',
    icon: <FileText className="size-6" />,
    size: 'medium' as const,
  },
  {
    title: 'Visual Audit',
    description:
      'Interactive charts show your degree progress across gen eds, credits, and major requirements',
    icon: <PieChart className="size-6" />,
    size: 'small' as const,
  },
  {
    title: 'Four-Year Planner',
    description:
      'Map out every semester, from freshman year to graduation, and stay on track',
    icon: <Calendar className="size-6" />,
    size: 'large' as const,
  },
];


export function BentoGrid1() {
  const containerVariants = {
    hidden: {},
    visible: {
      transition: {
        staggerChildren: 0.12,
        delayChildren: 0.1,
      },
    },
  };

  return (
    <section className="w-full px-4 py-16">
      <div className="mx-auto max-w-6xl">
        <motion.div
          className="grid grid-cols-6 gap-4"
          variants={containerVariants}
          initial="hidden"
          animate="visible"
        >
          {items.map((item, index) => (
            <BentoGridItem
              key={index}
              title={item.title}
              description={item.description}
              icon={item.icon}
              size={item.size}
              className={cn(
                item.size === 'large'
                  ? 'col-span-6 lg:col-span-4'
                  : 'col-span-6 sm:col-span-3'
              )}
            />
          ))}
        </motion.div>
      </div>
    </section>
  );
}
