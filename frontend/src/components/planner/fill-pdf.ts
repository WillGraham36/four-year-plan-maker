...
export default async function fillPDFForm({
  ...
  const existingPdfBytes = await fetch(
    "/cmsc-graduation-plan-template.pdf",
  ...
  academicYears.forEach((academicYear, yearIndex) => {
    academicYear.semesters.forEach((semester) => {
      ...
      const [startCourse, startGenEd, startCredit] = startIndicesForSemester[
        semesterKey
      ] || [0, 0, 0];
      ...
      courses.forEach((course, courseIndex) => {
        ...
        form
          .getTextField(
            courseField === 0 ? "Course" : `Course_${courseField + 1}`,
          )
        ...
        form
          .getTextField(genEdField === 0 ? "GenEd" : `GenEd_${genEdField + 1}`)
          .setText(cleanedGenEds);
        form
          .getTextField(creditField === 0 ? "Cr" : `Cr_${creditField + 1}`)
          .setText(course.credits.toString());
      });
      ...
      if (["FALL", "SPRING", "SUMMER"].includes(semester.term)) {
        ...
        const creditIndex =
          indicesForTotalCreditsPerSemester[semesterKey as SemesterKeyNoWinter];
        if (typeof creditIndex !== "undefined") {
          form
            .getTextField(`Cr_${creditIndex + 1}`)
            .setText(totalCredits.toString());
        }
      }
    });
  });
  ...
  genEdRequirements.map(({ courseId, semesterName, transferCreditName }, i) => {
    ...
    form
      ...
      .setText(transferCreditName ? transferCreditName : courseId);
    form
      .getTextField(i === 0 ? "Semester" : `Semester_${i + 1}`)
      .setText(courseName === "Transfer" ? "---Transfer---" : courseName);
  });
