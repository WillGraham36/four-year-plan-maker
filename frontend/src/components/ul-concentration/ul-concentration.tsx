...
const UpperLevelConcentrationContainer = ({
  ...
  const [concentration, setConcentration] =
    useState<string>(initialConcentration);
  const [coursePickerOpen, setCoursePickerOpen] = useState(false);
  const [updatingCourseKey, setUpdatingCourseKey] = useState<string | null>(
    null,
  );
  const { ULCourses, refreshULCourses, completedSemesters } = useRequirements();
  const { updateULConcentration, addCustomULCourse, removeCustomULCourse } =
    useCourseApi();
  const totalCredits = ULCourses.reduce(
    (total, course) => total + course.credits,
    0,
  );
  const availableCustomCourses = useMemo(() => {
    const customCourseIds = new Set(
      ULCourses.filter((course) => course.custom).map(
  ...
  const onRemoveCustomCourse = async (
    courseId: string,
    term: Term,
    year: number,
  ) => {
    ...
    <div className="flex flex-col rounded-lg border w-full h-min overflow-hidden bg-card shadow-md">
      <div className="flex items-center justify-between w-full border-b py-1.5 px-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-semibold text-lg">
            ...
            <span className="text-muted-foreground text-sm md:text-base ml-1">
              ({totalCredits} / 12 credits)
            </span>
          </p>
        </div>
        <ULCCombobox
          value={concentration}
          setValueStateAction={async (newConcentration) => {
      ...
      <div className="grid grid-cols-[1fr_2fr_7rem] text-xs md:text-sm text-muted-foreground items-center">
        {ULCourses.map((course, i) => {
          const isCompleted = completedSemesters.some(
            (sem) =>
              sem.term === course.semester.term &&
              sem.year === course.semester.year,
          );
          const courseKey = getCourseSemesterKey(
            course.courseId,
            course.semester.term as Term,
            course.semester.year,
          );
          return (
            <Fragment key={courseKey}>
              <div
                className={`w-full px-3 py-1 flex items-center justify-between gap-2 ${i !== ULCourses.length - 1 ? "border-b" : ""} ${isCompleted ? "bg-green-500/15 dark:bg-green-800/15" : ""}`}
              >
              ...
              <p
                className={`border-x w-full h-full flex items-center px-3 py-1 bg-background ${i !== ULCourses.length - 1 ? "border-b" : ""} ${isCompleted ? "bg-green-500/15 dark:bg-green-800/15" : ""}`}
              >
                {termYearToString(course.semester.term, course.semester.year)}
              </p>
              <p
                className={`w-full py-1 h-full flex items-center justify-center bg-background ${i !== ULCourses.length - 1 ? "border-b" : ""} ${isCompleted ? "bg-green-500/15 dark:bg-green-800/15" : ""}`}
              >
                {course.credits}
              </p>
            </Fragment>
        ...
        {ULCourses.length === 0 && (
          <p className="col-span-3 text-center py-2">
            No courses found for this concentration
          </p>
        )}
      </div>
      ...
      <CommandDialog
        ...
        title="Add Custom Upper Level Course"
      >
        <CommandInput placeholder="Search upper-level courses..." />
        <CommandList>
          <CommandEmpty>
            No available 300+ level planned courses found
          </CommandEmpty>
          <CommandGroup heading="Planned Courses">
            {availableCustomCourses.map(({ course, term, year }) => {
              ...
              return (
                <CommandItem
                  ...
                  onSelect={() =>
                    onAddCustomCourse(course.courseId, term, year)
                  }
...
const getCourseSemesterKey = (courseId: string, term: Term, year: number) =>
  `${courseId}-${term}-${year}`;
